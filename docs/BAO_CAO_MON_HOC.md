...
- MQTT broker Mosquitto 2.x (2 instance: Broker 1 port 1883, Broker 2 port 1884).
- Nginx làm reverse proxy – điểm vào duy nhất của hệ thống.
- Docker Compose để triển khai toàn bộ hệ thống (6 services).
...
│  │   Port: 3308       │◄──│   Port: 3000        │◄──│   /api/* → backend                 │
│  └────────────────────┘   └────────────────────┘   │   /*    → frontend                  │
│                                                     └─────────────┬───────────────────────┘
└──────────────────────────────────────────────────────────────────────────────────────────┘
                                                                    ▲
...
[ESP32 Sensor]                                                [Browser Dashboard]
     ...
     │ B2. HMAC-SHA256(sensor_secret, "id:ts")                         │     hiển thị data mới
...
| B3 | Sensor | Publish JSON `{sensor_id, timestamp, hmac, temperature, humidity}` lên topic `local/sensors/{sensor_id}/data` → **Broker 1 :1883** | QoS mặc định của PubSubClient; LED gửi nhấp nháy 200ms |
| B4 | Broker 1 | Chuyển tiếp message đến mọi subscriber khớp wildcard `local/sensors/+/data` | Broker không kiểm tra nội dung payload |
...
| B9 | Gateway | **MQTT publish** lên topic `gateway/<GW_DEVICE_ID>/data` → **Broker 2 :1884** (hằng `GATEWAY_DATA_TOPIC` trong `config_gw.h`). Payload: outer fields (gw_id, gw_ts, gw_hmac, gateway_ip) + field `sensor_payload` lồng bên trong (sensor_id, sn_ts, sn_hmac, sensor_ip, data) | LED forward nhấp nháy khi publish OK; lỗi mạng MQTT → retry ở vòng loop kế tiếp |
| B10 | Broker 2 | Chuyển tiếp message đến backend đang subscribe wildcard `gateway/+/data` | Broker 2 bật `log_dest topic` — $SYS log IP gateway kết nối |
| B11 | Backend (mqttDataService.ts) | Nhận message qua MQTT callback. Xác thực **độc lập** cả gateway và sensor: verify gw_hmac (DB lookup gateway_secret) rồi verify sn_hmac (DB lookup sensor_secret) | Một trong hai sai → log lỗi, tăng fail_count, không lưu |
| B12 | Backend | `INSERT INTO sensor_data (device_id, gateway_id, payload, received_at)` | `payload` lưu dạng JSON `{temperature, humidity}` |
| B13 | Backend | `UPDATE devices SET last_seen = NOW(), fail_count = 0 WHERE id IN (sensor.id, gateway.id)` | Làm cơ sở tính online/offline ở dashboard |
| B14 | Backend | Ghi `audit_log` với `event_type = 'DATA_RECV'` | Phục vụ truy vết & threat model |
| B15 | Dashboard | SWR poll `GET /api/devices` mỗi **10 giây** (không dùng WebSocket/socket.io) | Cập nhật bảng thiết bị, biểu đồ Recharts, trạng thái online/offline |
...
| Firmware | Sensor / Gateway / Build / Cảm biến | ESP32 DOIT DevKit V1 (cả 2 board) / PlatformIO / DHT22 | — |
| Firmware | Mật mã / Đồng bộ thời gian | mbedTLS (HMAC-SHA256) / NTP (pool.ntp.org, UTC+7) | — |
| Hạ tầng | Proxy / Broker / Container / DBMS | Nginx (Alpine) / Mosquitto MQTT (×2) / Docker Compose / MySQL | port 80 / port 1883 & 1884 / 6 services / port 3308→3306 |
...
| Data | `POST /api/device/data` | HMAC-SHA256 (2 lớp) | HTTP fallback endpoint. **Luồng chính qua MQTT** (`mqttDataService.ts`). Rate limit 60/phút/IP |
...
**Payload MQTT `gateway/{gw_id}/data`** (cũng là payload của HTTP fallback `POST /api/device/data`):
```json
{
...
Sensor              Gateway                          Backend (validateDevice)
  ...
  │                    │  (MQTT là one-way, không có HTTP reply) │
```
...
**Phần cứng:** ESP32 DOIT DevKit V1 (ESP32-WROOM-32), dual-core Xtensa LX6 @240MHz — không có cảm biến, đóng vai trò security validator và MQTT forwarder.

**Luồng khởi động:** kết nối WiFi (retry 20 lần) → đồng bộ NTP → kết nối **Broker 1 :1883** → `subscribe("local/sensors/+/data")` → kết nối **Broker 2 :1884** (kênh publish lên backend). Gateway duy trì 2 kết nối MQTT song song.
...
[6] Build payload JSON: outer fields (gateway_id, gw_timestamp, gw_hmac, gateway_ip)
      + field `sensor_payload` lồng bên trong (sensor_id, sn_timestamp, sn_hmac, sensor_ip, data)

[7] MQTT publish lên topic `GATEWAY_DATA_TOPIC` = `"gateway/<GW_DEVICE_ID>/data"`
      └─ Publish OK             → LED_FWD nhấp nháy (forward thành công)
      └─ Mất kết nối MQTT      → LOG("MQTT publish failed"), reconnect qua maintainMQTT(),
                                   chờ message tiếp theo từ sensor
      (Backend subscribe wildcard `gateway/+/data` nhận message và xử lý trong `mqttDataService.ts`)
```
...
**Realtime:** SWR auto-refresh (`refreshInterval: 10000`) danh sách thiết bị mỗi 10 giây — không dùng WebSocket hay socket.io; `online` = `last_seen > NOW() - 60s`. Nginx forward `Upgrade`/`Connection: upgrade` headers chỉ dành cho HMR (Next.js dev mode), không phải realtime data.
...
services:
  ...
  mqtt-broker-1:  { image: eclipse-mosquitto:2, ports: ["1883:1883"] }   # Sensor↔Gateway
  mqtt-broker-2:  { image: eclipse-mosquitto:2, ports: ["1884:1883"] }   # Gateway→Backend
...
| **8. Container hóa & reverse proxy** | Viết `Dockerfile.dev` cho backend/frontend, `docker-compose.yml` (6 services: 2 MQTT broker riêng biệt), `nginx.conf` (routing `/api/*` và `/*`) | Docker Compose, kiểm thử bằng `docker compose up -d --build` rồi gọi `/api/health` | Toàn bộ hệ thống khởi động bằng 1 lệnh duy nhất |
...
**Khởi động môi trường:** `docker compose up -d --build`, kiểm tra 6 service (`iot-mysql`, `iot-mqtt-broker-1`, `iot-mqtt-broker-2`, `iot-backend`, `iot-frontend`, `iot-nginx`) đều `running`/`healthy`. Truy cập `http://localhost` (Nginx), `http://localhost/api/health`.
...
**Điểm yếu tiềm tàng:** MQTT Broker 1 (port 1883) và Broker 2 (port 1884) không TLS → dữ liệu plain text trên LAN. **Biện pháp:** HMAC authentication khiến dữ liệu bị sniff vẫn không thể sửa mà không bị phát hiện (HMAC sẽ không khớp); kiến trúc 2 broker tách biệt làm giảm blast radius khi 1 kênh bị compromise; Helmet HSTS ép browser dùng HTTPS; Nginx là điểm duy nhất cần bổ sung TLS certificate. **Còn tồn tại:** cả 2 MQTT broker chưa TLS — nên triển khai Mosquitto TLS trong production.
...
*MQTT Broker 1 (:1883) và Broker 2 (:1884) chưa TLS trên LAN — cần Mosquitto TLS cho cả 2 broker trong production.
...
7. **Container hóa đầy đủ:** Docker Compose 6 services (bao gồm 2 MQTT broker độc lập) giúp triển khai nhất quán, giảm lỗi "works on my machine".

## 5.4. Hạn chế

| Hạn chế hiện tại | Hướng cải tiến | Độ ưu tiên |
|-----------------|----------------|:----------:|
| MQTT Broker 1 (:1883) và Broker 2 (:1884) chưa TLS, cho phép anonymous access | Cấu hình Mosquitto TLS + client certificate cho cả 2 broker | Cao |
...
- Bật TLS cho cả 2 MQTT broker (Mosquitto Broker 1 & Broker 2) và HTTPS cho Nginx/API/dashboard.
...
├── nginx/nginx.conf
├── mosquitto/broker1/mosquitto.conf    # Broker 1 — Sensor↔Gateway (:1883)
├── mosquitto/broker2/mosquitto.conf    # Broker 2 — Gateway→Backend (:1884)
├── docs/
