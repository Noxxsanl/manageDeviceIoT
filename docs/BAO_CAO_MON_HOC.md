...
```
IoT Device  ──MQTT──►  Mosquitto  ──MQTT──►  Backend Server  ◄──►  Dashboard
(Sensor ESP32)           Broker       Gateway        (Express + MySQL)     (Next.js)
                     (Port 1883)   (ESP32 DOIT V1)       ◄── Nginx (Port 80) ──► Browser
```
...
│                                                                                        │
│  ┌─────────────┐    MQTT     ┌───────────────┐   MQTT Publish   ┌───────────────────┐  │
│  │ Sensor Node │  ─────────► │ Gateway Node  │  ──────────────► │  Mosquitto Broker │  │
│  │  ESP32 DOIT │  local/sens │  ESP32 DOIT   │  gateway/{id}/  │   Port: 1883      │  │
│  │  V1 DHT22   │  ors/+/data │  DevKit V1    │  data           └────────┬──────────┘  │
│  └─────────────┘             └───────────────┘          │                         │   │
│                                                          │ Backend subscribe        │   │
│                                                          │ gateway/+/data           │   │
│                                                          ▼                          │   │
│  ┌──────────────┐  subscribe  ┌──────────────┐  ┌───────────────────┐              │   │
│  │  Mosquitto   │ ──────────► │   Backend    │  │  Nginx (Port 80)  │              │   │
│  │  Broker      │             │  Express.js  │  │  /api/*→backend   │              │   │
│  │  Port: 1883  │             │  Port: 5000  │  │  /*→frontend      │              │   │
│  └──────────────┘             └──────┬───────┘  └────────┬──────────┘              │   │
│                                      │                   │                          │   │
│                              ┌───────▼───────┐   ┌───────┴───────┐                 │   │
│                              │  MySQL 8.0    │   │  Frontend     │                 │   │
...
[ESP32 Gateway]                                                   │
     ...
     │ B9. MQTT publish: gateway/{gw_id}/data → Mosquitto           │
     ↓                                                            │
[Mosquitto MQTT Broker]                                           │
...
| B9 | Gateway | **MQTT publish** lên topic `gateway/<GW_DEVICE_ID>/data` (hằng `GATEWAY_DATA_TOPIC` trong `config_gw.h`). Payload: outer fields (gw_id, gw_ts, gw_hmac, gateway_ip) + field `sensor_payload` lồng bên trong (sensor_id, sn_ts, sn_hmac, sensor_ip, data) | LED forward nhấp nháy khi publish OK; lỗi mạng MQTT → retry ở vòng loop kế tiếp |
| B10 | Backend (mqttDataService.ts) | Nhận message qua MQTT callback. Xác thực **độc lập** cả gateway và sensor: verify gw_hmac (DB lookup gateway_secret) rồi verify sn_hmac (DB lookup sensor_secret) | Một trong hai sai → log lỗi, tăng fail_count, không lưu vào B11 |
...
| B14 | Backend → MySQL | `audit_log` ghi `DATA_RECV`, xong. **Không có HTTP response** — luồng MQTT là one-way publish, gateway không chờ reply từ backend. | Backend log: `[mqttData] saved id=N from ESP32-SN-... via ESP32-GW-...` |
...
Sensor              Gateway                          Backend (validateDevice)
  ...
  │                    │            "gateway_id:ts2")           │
  │                    │─── MQTT publish gateway/{gw_id}/data ─►│  (Mosquitto broker)
