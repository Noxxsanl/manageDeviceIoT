...
| **Giám sát và truy vết** | Audit log 7 event types trong database |
...
| Audit log | `DELETE /api/audit-log/data-recv` | `verifyJWT` + role check inline | admin, operator | `audit.ts` |
...
| 13 | Audit log bảo mật | Hoàn thành (ngoài yêu cầu gốc) | 7 event types: DATA_RECV, GATEWAY_AUTH_FAIL, SENSOR_AUTH_FAIL, DEVICE_BLOCKED, DEVICE_REGISTER, DEVICE_STATUS_CHANGE, DEVICE_DELETE |
...
- Audit log 7 event types cho truy vết bảo mật
