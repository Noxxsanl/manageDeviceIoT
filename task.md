...
> **⚡ Cập nhật kiến trúc 2026-06-17:** Hệ thống đã nâng cấp từ **1 MQTT broker** lên **2 MQTT broker** (dual-broker).
> - **Broker 1 :1883** — Sensor Node ↔ Gateway Node (local LAN)
> - **Broker 2 :1884** — Gateway Node ↔ Backend (Gateway publish, Backend subscribe)
> - Gateway không còn dùng HTTP POST lên Backend — đã chuyển sang MQTT Publish sang Broker 2.
> - Backend đọc dữ liệu qua `mqttDataService` (subscribe `gateway/+/data` trên Broker 2), không còn endpoint HTTP `/api/device/data`.
...
- Tạo file `.env.example` với các biến: `DB_HOST`, `DB_USER`, `DB_PASS`, `DB_NAME`, `JWT_SECRET`, `MQTT_HOST`, `MQTT_PORT`, `PORT`
  > _(Triển khai thực tế: `MQTT_PORT=1884` cho Broker 2; Docker override sang `mqtt-broker-2:1883`)_
- Tạo `.gitignore` (loại trừ `.env`, `node_modules`, `config.h`)
- Cấu hình `docker-compose.yml` với **4 service**: `mysql:8.0`, `mqtt-broker-1` (Broker 1 :1883), `mqtt-broker-2` (Broker 2 :1884→1883), `backend`
  > _(Triển khai thực tế: 2 service broker riêng biệt thay vì 1 service mosquitto)_
- Cấu hình Mosquitto:
  - `mosquitto/broker1/mosquitto.conf` — listener 1883, allow_anonymous true (Sensor↔Gateway)
  - `mosquitto/broker2/mosquitto.conf` — listener 1883, log_dest topic (Gateway↔Backend, cho $SYS IP tracking)
- Kiểm tra: `docker-compose up` → MySQL, Broker 1, Broker 2 khởi động thành công
...
- Viết `config_gw.h`: `GW_DEVICE_ID`, `GW_SECRET_KEY`, `MQTT_BROKER1_HOST/PORT` (1883), `MQTT_BROKER2_HOST/PORT` (1884)
  > _(Triển khai thực tế: không dùng `BACKEND_URL` vì gateway dùng MQTT, không HTTP POST)_
- Viết `mqtt_client.cpp` — **dual PubSubClient pattern**:
  - `mqttSubClient` → Broker 1 :1883 — subscribe `local/sensors/+/data`
  - `mqttPubClient` → Broker 2 :1884 — publish `gateway/{id}/data`
- Viết `forwarder.cpp`:
  ...
  - Build payload đầy đủ: `{gateway_id, gw_timestamp, gw_hmac, gateway_ip, sensor_payload:{sensor_id, sn_ts, sn_hmac, sensor_ip, data}}`
  - `mqttPubClient.publish("gateway/GW_ID/data", payload)` → **Broker 2**, không còn HTTP POST
- Viết `main.cpp`: setup WiFi + NTP + `mqttClientSetup()`, loop: `mqttClientMaintain()` duy trì cả 2 client
- **Integration Test toàn luồng:** Sensor → Broker 1 → Gateway (xác thực SN) → Broker 2 → Backend subscribe (xác thực GW+SN) → MySQL → Dashboard
