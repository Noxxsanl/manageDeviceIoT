...
| `DELETE` | `/api/devices/:id` | Xoá thiết bị (cascade sensor_data, tokens, audit) | admin |
...
| `DELETE` | `/api/audit-log/data-recv` | Xoá toàn bộ DATA_RECV logs | admin |
| `DELETE` | `/api/audit-log/by-type` | Xoá log theo loại event | admin |
| `DELETE` | `/api/audit-log/bulk` | Xoá nhiều log theo ID | admin |
...
audit_log
  ...
  device_id     INT UNSIGNED NULL FK → devices.id   (ON DELETE SET NULL)
...
| Event | Ý nghĩa | Visible by |
|---|---|---|
| `DATA_RECV` | Nhận dữ liệu cảm biến thành công | admin, operator, viewer |
| `DEVICE_REGISTER` | Thiết bị đăng ký mới | admin, operator, viewer |
| `DEVICE_BLOCKED` | Thiết bị bị auto-block | admin, operator, viewer |
| `DEVICE_STATUS_CHANGE` | Thay đổi trạng thái | admin, operator, viewer |
| `GATEWAY_AUTH_FAIL` | Lỗi xác thực HMAC gateway | admin, operator |
| `SENSOR_AUTH_FAIL` | Lỗi xác thực HMAC sensor | admin, operator |
| `REPLAY_ATTACK` | Timestamp ngoài cửa sổ ±300s | admin, operator |
| `PRIVILEGE_ESCALATION` | Thiết bị sai device_type | admin, operator |
| `DEVICE_DELETE` | Thiết bị bị xóa | admin |
...
| `/logs` | Logs | Stub page — hiện chưa có dữ liệu (`<LogTable logs={[]} />`) |
